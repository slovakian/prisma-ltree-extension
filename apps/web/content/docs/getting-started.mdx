...
Declare `ltree` columns in either authoring lane — `contract.prisma` (PSL) or `contract.ts` (TypeScript). Both emit the same compiled contract; see [Authoring Contracts](/docs/authoring) for the PSL surface (`ltree.Ltree()` / `ltree.LtreeArray()`) and a side-by-side comparison.

...
PostgreSQL `ltree` brings hierarchical tree data to **Prisma Next**. This extension pack provides typed codecs, query operators, and a baseline migration for tree-structured data — without raw SQL.

Author your contract in either lane and get the same compiled result:

- **PSL** — `contract.prisma` with `ltree.Ltree()` / `ltree.LtreeArray()`
